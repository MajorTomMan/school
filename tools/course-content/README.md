...
APK 只包含课程数据结构、解析器、同步缓存、页面渲染器、可视化与确定性算法。教材目录、章节、课程正文、例题、小结和习题全部通过远端清单同步到应用私有缓存；云端不可用时使用上一次有效缓存，没有缓存时显示课程数据缺失页面。
...
同步完成后，客户端从 `course.json` 构建教材、章节和小节导航。APK 不包含任何替代目录或课程正文，因此课程包必须提供完整且稳定的 ID、标题、页码和页面块。
